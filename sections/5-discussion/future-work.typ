...
The current approach to estimating obstacles assumes a static environment where all moving entities can be communicated with by each robot. It also requires a pre-known #acr("SDF") representation of the entire environment, which is suitable for logistic facilities but not for scenarios where the environment is partially or fully unknown. There exists work on #acr("VIO") based #acr("SLAM") that uses factorgraphs as the belief optimization algorithm@factor-graphs-exploiting-structure-in-robotics. Work into combining these two steps; localization and estimation with path planning, into a joint factorgraph could prove beneficial as the information contained in the assembled environment map could substitute for the existing obstacle factors. Another limitation is that dynamics factors assume the robot is not subjected to any non-holonomic constraints. A common choice for ground robots is to use differential drive dynamics, that has constraints on the ability to do lateral movements@diffential-drive-constraints. A dynamic factor that captures this hard constraint would be necessary for moving towards real-world applicability. Another issue is that the horizon state is updated to move closer to the goal pose regardless of the current state, which infrequently can lead to a robot getting stuck. More sophisticated logic is needed to advance the horizon state in these situations. Finally, investigating the capabilities of the algorithm with a dynamic number of variables, depending on the desired velocity, could improve computational efficiency.
Finally, it would be valuable to investigate the algorithm's capabilities for factor graphs with a dynamic number of variables throughout the graph's lifetime. For example, if the number of variables depends on the desired velocity, more variables and a longer future horizon might be beneficial for accelerating the robot and increasing its speed. Conversely, at a lower desired velocity, fewer variables could suffice, making the process less computationally intensive.
...
Extending the current algorithm to a three-dimensional context would be interesting to explore to as it would extend applicability for use cases where quad-rotor #acrpl("UAV") are deployed. With a state space comprized of 12 states@tahir2019state#footnote[Position $x,y,z$, linear velocity $dot(x), dot(y), dot(z)$, orientation $phi.alt, theta, psi$ and angular velocity $dot(phi.alt), dot(theta), dot(psi)$.] instead of four, the multivariate normal distribution messages exchanged during variable and factor iteration would be $7$ times larger in terms of bytes, and thereby a lot more computationaly costly, which could limit the scalability in highly connected cases. Both the dynamic factor and obstable factor would have to be redesigned, with a new static environment representation to replace the #acr("SDF") representation used in 2D.
