...
The *Distance Travelled* metric, see @f.circle-experiment-distance-travelled, a somewhat similar discrepancy in results when comparing #acr("MAGICS") to the original paper. Even with the best performing configuration of #lm3-th5.s for #acr("MAGICS"), it still has a descrepency when comparing to the original paper. #acr("MAGICS") achieves a higher variance, and faster increasing trend upwards as $N_R$ is increased. One similarity between the two, is that *Distance Travelled* seems to increase linearly proportional to $N_R$.
...
In the *Communications Failure* scenario, the communications radius $r_C$, was tested for values ${20, 40, 60, 80}m$. Here this thesis provides these results for #lm3-th13.n and #lm3-th5.n for #acr("MAGICS"), and #lm3-th13.n for the original `gbpplanner` code; while comparing to the #gbpplanner paper. This is the experiment that makes it clear how unattainable the results of @gbpplanner. #acr("MAGICS") performs relatively well when looking at the makespan metric#note.j[check if it is for both configs], but at the cost of a large abount of interrobot collisions, see @t.comms-failure-experiment. The results of #acr("MAGICS") can be validated against the `gbpplanner` code, which achieves reciprocally similar results. That is, instead of a trade-off for a large number of collissions, the `gbpplanner` code takes a lot longer to solve the problem. Unfortunately it was not possible to count the number of collissions for `gbpplanner`, but looking at it, it seems to be nearly 0 for all levels of communication failure#footnote[See a demonstration of this at #link("https://youtu.be/MsuO7i2gRaI", "YouTube: Master Thesis - GBP Planner Communication Failure Rate 0.9")], similar to the results presented in the #gbpplanner paper. Nonetheless, the makespan for #acr("MAGICS") is much lower, and approaches @gbpplanner for a target speed of $10m"/"s$. For a target speed of $15m"/"s$ the makespan descrepancy is higher, starting at $tilde.op 135%$ of the #gbpplanner result with no communication failure, and increasing to $tilde.op 165%$ for a failure rate of $gamma = 70%$. An important note is that the results for $gamma = {80, 90}%$ are not available for #acr("MAGICS"), as the simulation was not able to complete within a reasonable time frame. It is the case that with such low communication the robots simply end up holding each other up, and the simulation approaches convergence incredibly slowly.
