...
Patwardhan #etal presents the idea of using factor graphs for multi-agent path planning in _Distributing Collaborative Multi-Robot Planning with Gaussian Belief Propagation_@gbpplanner. In their formulation each robot has a factor graph where variables are possible future states spread out over a forward time window. Four different factors are used; pose, obstacle, dynamics and inter-robot. Each one encodes a different constraint needed for efficient and collision-free paths. As this thesis aims to reimplement and extend the work by Patwardhan #etal a detailed explanation of each factor will be omitted here, and instead be presented in detail in @s.m.factor-graph. The most important factor to mention here is the interrobot factor which is used to facilitate communication between robots. These factors are ephemeral and are added whenever two robots are within communication range of each other. They add shared constraints of minimum tolerable distance between variables in each robots factorgraph. Their approach incorporates #acr("GBP") as the inference algorithm used to determine marginal distributions for the future poses which best satisfy all constraints. Robots exchange messages between each other where the payload are marginalised multivariable gaussian distributions that describe a robots belief about the other robots future states.
