...
#note.jo[this is new, please check]The underlying problem addressed by this thesis is the inefficiency and limitations in existing multi-agent path planning systems, particularly in complex environments. Current systems often struggle with issues related to collision avoidance, especially in context of multi-agent collision avoidance, and effective navigation through constrained and complex spaces. These challenges are exacerbated by the lack of robust, flexible, and extendable simulation frameworks that can accurately reproduce and enhance existing path planning algorithms. Furthermore, in the space of #acr("GBP") planning collaborative planning algorithms, which is still a quite new field, there is lack of automated global planning capabilities. The problem of integrating global planning with #acr("GBP") comes with further challenges, and it is important to consider possible solutions, in such a way that it doesn't compromise the performance of the system. This thesis seeks to solve these problems by developing a comprehensive solution that enhances the usability, functionality, and scalability of multi-agent path planning systems in the context of #acr("GBP") algorithms. The solution will be implemented in a simulated environment, and evaluated based on a set of predefined metrics.
