...
#let snippet = sourcecode.with(
  numbering: none,
  frame: std-block.with(
    inset: 2pt,
    radius: 3pt,
  )
)
...
#let todo(txt) = boxed(
  color: theme.maroon.lighten(0%),
  fill: theme.maroon.lighten(80%),
  [*Todo:* #txt]
)

#let jens(txt) = boxed(
  color: theme.teal.lighten(0%),
  fill: theme.teal.lighten(80%),
  [*Jens:* #txt]
)

#let kristoffer(txt) = boxed(
  color: theme.green.lighten(0%),
  fill: theme.green.lighten(80%),
  [*Kristoffer:* #txt]
)

#let pernille(txt) = boxed(
  color: theme.mauve.lighten(0%),
  fill: theme.mauve.lighten(80%),
  [*Pernille:* #txt]
)

#let bastian(txt) = boxed(
  color: theme.blue.lighten(0%),
  fill: theme.blue.lighten(80%),
  [*Bastian:* #txt]
)

#let emil(txt) = boxed(
  color: theme.pink.lighten(0%),
  fill: theme.pink.lighten(80%),
  [*Emil:* #txt]
)

#let magnus(txt) = boxed(
  color: theme.peach.lighten(0%),
  fill: theme.peach.lighten(80%),
  [*Magnus:* #txt]
)

#let daniel(txt) = boxed(
  color: theme.yellow.lighten(0%),
  fill: theme.yellow.lighten(80%),
  [*Daniel:* #txt]
)
...
#let boxed-enum(
  ...
  color: color,
  fill: fill,
  prefix + numbers.pos().map(str).join(".") + "." + suffix,
  weight: 900
)
