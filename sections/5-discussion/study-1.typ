...
The *Distance Travelled* metric, see @f.circle-experiment-distance-travelled, a somewhat similar discrepancy in results when comparing #acr("MAGICS") to the original paper, however, the #acr("MAGICS"), even with #lm3-th5.s does not come close to the results of the original paper. #acr("MAGICS") achieves a much higher variance, and faster increasing trend upwards as $N_R$ is increased. One similarity between the two, is that *Distance Travelled* seems to increase linearly proportional to $N_R$. #att[Again, we see that the `gbpplanner` code achieves very similar results to #acr("MAGICS").] The *Makespan* metric reinforces this observation, see @f.obstacle-experiment-makespan.
