...
#{
  ...
  table.header([Name], [Description], [Space Complexity]),
  table.hline(),
  [`Graph`], [Uses an _adjacency list_ to store nodes], [],
  [`StableGraph`], [Similar to `Graph`, but it keeps indices stable across removals.], [],
  [`GraphMap`], [Uses an adjacency list, but instead of storing nodes sequentially it uses generated node identifiers as keys into a hash table, where the value is a list of the nodes connected edges], [],
  [`MatrixGraph`], [Uses an _adjacency matrix_ to store nodes.], [$O(|V^2|)$],
  [`CSR`], [#acr("CSR") but it uses a sparse adjacency matrix to store nodes], [$O(|E| + |V|)$],
