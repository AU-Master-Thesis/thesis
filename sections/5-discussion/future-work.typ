...
Current networking simulation is very simplistic with robots being able to communicate as long as they are with a given radius of each other and their radio antenna is turned on. Exchange of messages happens instantly and without delay in simulated time. All of which poorly reflects the complexities present in a real-scenario communication. Enhancements

- Delays could be added to messages sent sampled from a stochastic distribution derived from empirical data on multi-robot short distance radio communication.

to not overload the available bandwidth for other core tasks needing communication access.

- Try to make an estimate about the throughput of the system, in terms of messages per second, and try and estimate how much bandwidth is used. Based on the results, discuss what implications this would have for a real world system.
