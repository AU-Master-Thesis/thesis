...
// -------------------------------------------------------------------------------
Two additional experiments have been performed to test different parameters of the GBP algorithm not explored in the original work. Firstly, the effect of the number of internal and external iterations on the algorithm's convergence and performance was examined. A key finding is that having more internal iterations than external iterations is beneficial, especially when internal iterations are low. Generally, no more than 10 iterations of each are sufficient across various scenarios to achieve good results. Additionally, the information within each robot's factor graph can be overwhelmed in highly connected cases if insufficient internal message passes are executed. These insights are promising for deploying GBP on a broad range of robots with limited computational power.

Secondly, the order in which new information is passed between robots from interrobot factors does not significantly impact performance. Similar results were obtained for all five schedules considered, supporting the strong benefits of using GBP as an inference algorithm for multi-agent path planning. However, strong conclusive statements require real-world tests or more advanced simulations of networking conditions.
