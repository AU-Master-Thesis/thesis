...
\node[dot, minimum size=0.5 * \scale, draw=ctpMaroon] (dt1) at (ctib1) {};
\node[dot, minimum size=0.5 * \scale, draw=ctpMaroon] (dt2) at (ctib2) {};
\node[dot, minimum size=0.5 * \scale, draw=ctpMaroon] (dt3) at (ctib3) {};

\draw[csnake, draw=ctpMaroon] (t1.north) -- node[yshift=10 * \scale, pos=0.5, text=ctpMaroon] (time) {Prediction Horizon} (t3.north);

\tikzset{shift={(0, -50 * \scale)}}
